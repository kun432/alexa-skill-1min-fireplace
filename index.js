...
const BACKGROUND_IMAGE_URL = '1min-fireplace-bg.png';
const VIDEO_FILE_NAME = `fire_`;
const VIDEO_FILE_EXT = '.mp4';
const VIDEO_TITLE = "１分間暖炉動画";
const VIDEO_SUBTITLE = "この動画はpixabay.comの著作権フリーな動画を使用しています。";
const TITLE = '１分間暖炉';
const FILE_NUM = 5;

const WELCOME_MSG = 'このスキルでは、仕事の合間などに、すこーーーーしだけリラックスできる、暖炉の炎の動画をランダムで再生します。';
...
const PlayVideoIntentHandler = {
  ...
  handle(handlerInput) {
    if (supportsDisplay(handlerInput)) {
      ...
      let primaryText = new Alexa.RichTextContentHelper()
        .withPrimaryText('暖炉の炎でリフレッシュしたら、またがんばりましょう！リラックスしたくなったときは、いつでもどうぞ！')
        .getTextContent();
